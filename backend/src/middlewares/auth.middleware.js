...
export const authMiddleware = asyncHandler(async (req, res, next) => {
  try {
    ...
    if (decodedToken.userId) {
      ...
      console.log(`ATTEMPTING TO FIND USER BY UUID: ${decodedToken.userId}`);
      user = await User.findOne({ userId: decodedToken.userId }).select(
        "-passwordHash -refreshToken"
      );
      console.log(
        "Auth middleware - Found user by userId:",
      ...
      console.log(
        "Auth middleware - Found user by _id:",
        user ? user.email : "none"
      );
    }
    ...
    if (!user) {
      console.log(
        "Auth middleware - No user found with userId:",
        decodedToken.userId
      );
