document.addEventListener('DOMContentLoaded', function () {
    ...
    function setupEventListeners() {
        // Modal open/close
        createNoticeBtn.addEventListener('click', openCreateModal);
    ...
    async function handleCreateNotice(e) {
        ...
        const submitBtn = noticeForm.querySelector('.btn-submit');
        ...
        try {
            ...
            const response = await window.noticeAPI.createNotice({ title, content });

            if (response && response.success) {
                notices.unshift(response.data); // Add new notice from server response
                updateNoticeStats();
                filterAndRenderNotices();
