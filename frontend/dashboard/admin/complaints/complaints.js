...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function createComplaintTile(complaint) {
        ...
        tile.innerHTML = `
            ...
            <div class="complaint-tile-footer">
                ...
                <button onclick="openAssignModal('${complaint.reportId}')">
    ...
    window.openAssignModal = async function (reportId) {
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h2>Assign Report <br>${complaint.reportId}</h2>
                <button class="close-btn" onclick="closeModal()">&times;</button>
            </div>
            <div class="modal-body">
                <p>Loading available admins and departments...</p>
            </div>
        </div>
    `;
        document.body.appendChild(modalOverlay);
        ...
        try {
            // Fetch ALL users and all departments in parallel
            const [usersResponse, departmentsResponse] = await Promise.all([
                window.adminAPI.getAllUsers(), // Fetches all users
                window.adminAPI.getAllDepartments()
            ]);

            if (!usersResponse.success || !departmentsResponse.success) {
                throw new Error("Failed to load necessary data for assignment.");
            }

            // Filter the full user list to find only the department admins
            const departmentAdmins = usersResponse.data.filter(user => user.role === 'department_admin');

            const departments = departmentsResponse.data;

            // --- Now, build the full modal content with the fetched data ---
            modalOverlay.innerHTML = `
            <div class="modal-content">
                <div class="modal-header">
                    <h2>Assign Report <br>${complaint.reportId}</h2>
                ...
                <div class="modal-body">
                    <p style="background-color: var(--water-light); padding: 15px; border-radius: 6px; margin-bottom: 20px;">
                        <strong><i class="fas fa-info-circle"></i> Issue:</strong> ${complaint.title}
                    </p>
                    <form id="assign-report-form">
                        <div class="form-grid-2">
                            <div class="form-group">
                                <label for="adminSelector"><i class="fas fa-user-shield"></i> Assign To Admin:</label>
                                <select id="adminSelector" name="adminSelector" required>
                                    <option value="" disabled selected>Select an Admin</option>
                                    ${departmentAdmins.map(admin => `<option value="${admin.userId}">${admin.name}</option>`).join('')}
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="departmentSelector"><i class="fas fa-building"></i> Assign To Department:</label>
                                <select id="departmentSelector" name="departmentSelector" required>
                                    <option value="" disabled selected>Select a Department</option>
                                    ${departments.map(dept => `<option value="${dept.departmentId}">${capitalizeWords(dept.name)}</option>`).join('')}
                                </select>
                            </div>
                        </div>
                        <button type="submit" class="btn-primary" style="margin-top: 20px;">
                            <i class="fas fa-check"></i> Assign Report
                        </button>
                    </form>
                </div>
            </div>
        `;
            ...
            document.getElementById('assign-report-form').addEventListener('submit', async (e) => {
                e.preventDefault();
                const submitBtn = e.target.querySelector('button[type="submit"]');
                submitBtn.disabled = true;
                submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Assigning...';

                const assignedUserId = e.target.adminSelector.value;
                const departmentId = e.target.departmentSelector.value;

                if (!assignedUserId || !departmentId) {
                    alert('Please select both an admin and a department.');
                    submitBtn.disabled = false;
                    submitBtn.innerHTML = '<i class="fas fa-check"></i> Assign Report';
                    return;
                }

                try {
                    // This is the API call to assign the report
                    await window.reportsAPI.assignReport(reportId, { assignedToUserId: assignedUserId, departmentId: departmentId });

                    // Find the names for the success message
                    const adminName = departmentAdmins.find(a => a.userId === assignedUserId)?.name || 'Unknown Admin';
                    const departmentName = departments.find(d => d.departmentId === departmentId)?.name || 'Unknown Dept';

                    alert(`Report assigned successfully to: ${adminName} \nDepartment: ${departmentName}`);
                    ...
                    submitBtn.disabled = false;
                    await window.reportsAPI.assignReport(reportId, { assignedTo: assignedUserId, departmentId: departmentId });
            ...
            modalOverlay.querySelector('.modal-body').innerHTML = `<p style="color: red;">Error: ${error.message}</p>`;
        }
    };
